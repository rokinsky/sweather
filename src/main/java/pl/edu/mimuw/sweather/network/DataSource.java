...
public abstract class DataSource {
	...
	public Observable<? extends SWeatherEvent> dataSourceStream() {
		return dataSourceStream(DEFAULT_POLL_INTERVAL);
	}

	public Observable<? extends SWeatherEvent> dataSourceStream(Integer pollInterval) {
		/*
	...
	protected <T> Observable<Long> fixedIntervalStream(int pollInterval) {
		return Observable.interval(INITIAL_DELAY, pollInterval, TimeUnit.SECONDS, Schedulers.io());
	}

	protected abstract Observable<? extends DataEvent> makeRequest();

	protected <T> HttpClientRequest<ByteBuf> prepareHttpGETRequest(String url) {
		/*
	...
	private <T> Observable<SWeatherEvent> wrapRequest(Observable<T> observable) {
		/*
		...
		return observable.flatMap(ignore -> Observable.concat(Observable.just(new NetworkRequestIssuedEvent()), // emit
				...
				makeRequest().timeout(TIMEOUT, TimeUnit.SECONDS).doOnError(log::error) // if
																						...
																						// abort
																						// it
						.cast(SWeatherEvent.class).onErrorReturn(ErrorEvent::new), // and
																					// emit
																					// an
																					// error
																					// event
